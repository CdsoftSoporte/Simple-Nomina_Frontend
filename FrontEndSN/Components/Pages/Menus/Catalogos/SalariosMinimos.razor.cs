...
{
	public partial class SalariosMinimos
	{

		private List<SalarioMinimo> salarios;
		
		protected SalarioMinimo editModel = new();
		...
		private async Task CargarDatos()
		{
			try
			{
				...
				var request = new HttpRequestMessage(HttpMethod.Get, "api/SalarioMinimo/Listar");
				...
				{
					var resultado = await response.Content.ReadFromJsonAsync<RespuestaBase<List<SalarioMinimo>>>(
									new JsonSerializerOptions { PropertyNameCaseInsensitive = true }
					);
					salarios = resultado.Data ?? new();
		...
		private void MostrarModal(SalarioMinimo? item = null)
		{
			if (item == null)
			{
				editModel = new SalarioMinimo();
			...
			{
				editModel = new SalarioMinimo { id = item.id, a_partir_de = item.a_partir_de, importe = item.importe };
			}
			isModalVisible = true;
		}
		...
		private async Task Guardar()
		{
			try
			{
				...
				string url = esNuevo ? "api/SalarioMinimo/Agregar" : $"api/SalarioMinimo/Editar/{editModel.id}";
		...
		private async Task ConfirmarEliminar(SalarioMinimo item)
		{
			bool confirmed = await JS.InvokeAsync<bool>("notifier.confirm",
		    "¿Eliminar Registro?",
		     $"¿Deseas borrar el salario de {item.a_partir_de:dd/MM/yyyy} con importe de {item.importe}?");
			if (confirmed)
			{
		...
		private async Task Eliminar(int id)
		{
			try
			{
				...
				var request = new HttpRequestMessage(HttpMethod.Post, $"api/SalarioMinimo/Borrar/{id}");
				request.Headers.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", token);
