...
namespace FrontEndSN.Components.Pages.Menus.Catalogos
{
	public partial class Umi
	{
		...
		private List<Umis> umi;

		protected Umis editModel = new();
		...
		private async Task CargarDatos()
		{
			try
			{
				...
				var request = new HttpRequestMessage(HttpMethod.Get, "api/UMI/Listar");
				...
				{
					var resultado = await response.Content.ReadFromJsonAsync<RespuestaBase<List<Umis>>>(
									new JsonSerializerOptions { PropertyNameCaseInsensitive = true }
					);
					umi = resultado.Data ?? new();
		...
		private void MostrarModal(Umis? item = null)
		{
			if (item == null)
			{
				editModel = new Umis();
			...
			{
				editModel = new Umis { id = item.id, a_partir_de = item.a_partir_de, importe = item.importe };
			}
			isModalVisible = true;
		}
		...
		private async Task Guardar()
		{
			try
			{
				...
				string url = esNuevo ? "api/UMI/Agregar" : $"api/UMI/Editar/{editModel.id}";
		...
		private async Task ConfirmarEliminar(Umis item)
		{
			bool confirmed = await JS.InvokeAsync<bool>("notifier.confirm",
			"¿Eliminar Registro?",
			 $"¿Deseas borrar la UMI de {item.a_partir_de:dd/MM/yyyy} con importe de {item.importe}?");
			if (confirmed)
			{
		...
		private async Task Eliminar(int id)
		{
			try
			{
				...
				var request = new HttpRequestMessage(HttpMethod.Post, $"api/UMI/Borrar/{id}");
				request.Headers.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", token);
